  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <form action="{{ route("social.update") }}" method="POST" enctype="multipart/form-data">
        @method("PUT")
        @csrf
        <input type="text" id="social_id" class="d-none" name="social_id" value="">
        <div class="modal-body">
          <div class="d-flex flex-column gap-1 mb-3">
            <label for="social_label">Label</label>
            <input type="text" name="social_label" id="social_label" disabled class="form-control" autocomplete="off">
          </div>
          <div class="d-flex flex-column gap-1 mb-3">
            <label for="social_value">Username</label>
            <input type="text" name="social_value" id="social_value" class="form-control" autocomplete="off">
          </div>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
<script>
  ...
  function showData(id) {
    $.get(`/user/social/data/${id}`, function(response, status) {
      if (status == "success") {
        $("#social_id").val(response.data.id);
        $("#social_label").val(response.data.label);
