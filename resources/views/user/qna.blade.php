  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Update QnA</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <form action="{{ route("qna.update") }}" method="POST" enctype="multipart/form-data">
        @method("PUT")
        @csrf
        <input type="hidden" name="id" id="qna_id">
        <div class="modal-body">
          ...
          <div class="d-flex flex-column gap-1 mb-3">
            ...
            <textarea type="text" name="answer" rows="5" id="answer" class="form-control" autocomplete="off"></textarea>
          </div>
        </div>
...
<script>
  $("#table").DataTable({
    ...
    columns: [
      {data: "question", render: function(data, type, row, meta) {
        return data;
      }},
      {data: "answer", render: function(data, type, row, meta) {
        return data;
      }},
  ...
  function showData(id) {
    $.get(`/user/qna/data/${id}`, function(response, status) {
      if (status == "success") {
        $("#qna_id").val(response.data.id);
