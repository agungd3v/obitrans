...
<div class="row">
  <div class="col-12">
    <div class="card">
      <div class="card-body">
        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add">Tambah Kontak</button>
      ...
      <form action="{{ route("contact.store") }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <div class="d-flex flex-column gap-1 mb-3">
            ...
            <select name="label" id="label" class="form-control" onchange="changeLabel(this)">
              <option value="" selected hidden>Pilih label</option>
          ...
          <div class="d-flex flex-column gap-1 mb-3 zone d-none">
            <label for="zone">Zona</label>
            <input type="text" name="zone" id="zone" class="form-control" autocomplete="off" placeholder="Cth: Jakarta">
          </div>
          <div class="d-flex flex-column gap-1 mb-3">
            <label for="contact">Contact</label>
            <input type="text" name="value" id="contact" class="form-control" autocomplete="off">
          </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form action="{{ route("contact.update") }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="d-flex flex-column gap-1 mb-3 zone-update d-none">
            <label for="contact_zone">Zona</label>
            <input type="text" name="contact_zone" id="contact_zone" class="form-control" autocomplete="off" placeholder="Cth: Jakarta">
          </div>
          <div class="d-flex flex-column gap-1 mb-3">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <form action="{{ route("contact.delete") }}" method="POST">
        @method("DELETE")
        @csrf
        <input type="hidden" id="delete_contact_id" name="id" value="">
...
<script>
  ...
  function showData(id) {
    $("#contact_zone").val(null);

    $.get(`/user/contact/data/${id}`, function(response, status) {
      if (status == "success") {
        if (response.data.label != "Phone") {
          $(".zone-update").removeClass("d-none");
        } else {
          $(".zone-update").addClass("d-none");
        }
        $("#contact_zone").val(response.data.zone);
